...
# Create LDA model
print("starting lda")
lda_model = LatentDirichletAllocation(n_components=true_k, random_state=0)
...
kmeans_lda = KMeans(n_clusters=true_k, max_iter=100, n_init=5)
fit_and_evaluate(kmeans_lda, X_lda, name="KMeans on LDA")
from gensim.models.coherencemodel import CoherenceModel
from gensim.corpora import Dictionary
from gensim.models.ldamodel import LdaModel

# Create a gensim dictionary
gensim_dict = Dictionary([text.split() for text in descriptions])

# Create a gensim corpus
corpus = [gensim_dict.doc2bow(text.split()) for text in descriptions]

# Create gensim LDA model
gensim_lda_model = LdaModel(
    corpus=corpus, id2word=gensim_dict, num_topics=true_k, random_state=0
)

# Compute coherence score
coherence_model_lda = CoherenceModel(
    model=gensim_lda_model,
    texts=[text.split() for text in descriptions],
