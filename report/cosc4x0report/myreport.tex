...
\begin{abstract}
	...
	need to be produced, as such we need a way to automatically predict them. There is precedent to using machine
	...
	comparison between the different sources, to see if we can get an estimated accuracy for each of
	...
	metric, hopefully once version 4.0 begins to be used commonly, it will solve some of these issues.
...
The National Vulnerability Database (NVD~\cite{NVD}) takes CVEs and enriches them with CVSS data.
They are not the only place to do so, however in terms of research they are often the main or sole
data provider (E.g.\@ \cite{costa, nvd_example1, nvd_example2}).  I explored other options, the main
obvious one being the MITRE database~\cite{MITRE}, as it is the main database for CVEs and they also
have a decent chunk of CVEs enriched with CVSS scores. As a guideline to my investigation between
the two databases I used the same method as the paper \textit{Can the Common Vulnerability Scoring
	System be Trusted? A Bayesian Analysis}~\cite{bayes}. This paper tries to see how much different
data sources agree on the scoring of a CVE, and thereby gain insight into the potential of a ground
truth value. Unfortunately since that paper was released back in 2016, many of the databases they
compared are either unavailable or in archival status. However, following their method still allows
for insight between the two chosen databases, NVD and MITRE. This analysis shows that the databases
do fundamentally rate CVEs differently (see Figure~\ref{fig:counts}). The uncertainty between the
two can therefore be an indicator going forward when analysing generated CVE scores, as it is likely
that the model will also struggle in similar places to where the human evaluators did. In addition
to this analysis, I looked into the CVSS system itself. There have been many complaints laid against
CVSS (E.g.\@~\cite{ubiquitous,improving_cvss, time_to_change_cvss}), these will be discussed in
Section~\ref{discussion}, but broadly, CVSS is used for more than it is designed to do,
...
the vulnerability is exploited? In this case we have the library, PyDoc~\cite{pydoc}, which this links to the
polyfill.io CDN. Ideally there would be a part of the description that relates to every metric,
...
\texttt{Attack Complexity} metric, there is a reasonably large difference in how they are rated, MITRE rate a
lot more of the metrics with a \texttt{Low} score. This points to some of the difficulty with this kind of
...
Section~\ref{cvss_prediction} as much of the process around training and making inferences of CVSS
scores comes from this paper.

Jiang and Atif in the paper, \textit{An Approach to Discover and Assess Vulnerability Severity
	Automatically in Cyber-Physical Systems}~\cite{jiang}, create a novel pipeline for vulnerability
assessment. They used multiple data sources and a majority voting system to decrease the chance of
badly scored CVEs. This paper relates in that it shows a use case for multiple data sources, though
less so on the methods used.
...
However as we do want to add some initial belief to this, as the people producing scores are
...
outside of my strengths, I did some cursory exploration into if a doing this sort of analysis
...
library. The vulnerability was described as an integer overflow in \texttt{tool\_operate.c}, which could
result in undefined behaviour if a user entered a value exceeding the integer limit~\cite{curlcve}. Initially, this
vulnerability received a CVSS score of 9.8. Daniel Stenberg, the original founder of curl, disagreed
with this high score, arguing that it was a bug fixed back in 2019~\cite{curl1}. Despite his efforts, the CVE
(Common Vulnerabilities and Exposures) entry remained, and he eventually became part of the CVE
Numbering Authorities. Stenberg successfully lobbied to have the National Vulnerability Database
(NVD) lower the score to 3.3~\cite{curl2}. This example highlights the time-consuming nature of rectifying CVSS
scores and the potential for initial misratings.
...
This is process is inherently inaccurate, it is not a system designed to give definitive scores. If
we look at~\ref{formula} above (\ref{equation} shows the CVSS 3.1 base score formula for reference),
the formula which produces the score does not match exactly the experts decision. There is a lot of
rounding and approximation going on. All this is designed to make a system which is easy to use and
quick to complete by security professionals. There is a space for this, however this along with the
other reasons outlines the issues with CVSS as a sole metric for prioritization. Perhaps an option
...
EPSS is developed by FIRST, the same group who govern the CVSS standard. It takes a different take
...
page are to believed, it is a promising system (some of their findings Figure~\ref{fig:epss},
...
the extra data during model training is worth it, so far it has only been to the detriment of
...
confuse automated systems and reduce the overall reliability of the predicted scores. Additionally,
our comparison revealed that the current CVSS system, particularly version 3.1, has inherent flaws
that may not fully be addressed until the widespread adoption of version 4.0.
...
offers a promising solution to automate and enhance the accuracy of CVSS scoring. Future research
should focus on refining these models, and focusing on interpretability, whether through LLM
interpretability methods, or through more traditional databased clustering.
