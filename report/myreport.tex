...
\begin{itemize}
	...
	\item	K-Means Clustering: We start with a straightforward K-means clustering approach to get an initial
	      understanding of how vulnerability descriptions group together. We arbitrarily choose 8 clusters to
	      match the number of CVSS metrics, providing a baseline for further analysis.
...
\begin{itemize}
	...
	\item \textbf{CVSS Prediction Challenges:} Our experiments with CVSS prediction using a DistilBERT
	      model trained on combined NVD and MITRE data revealed unexpected challenges. Contrary to our
	      hypothesis, data augmentation led to decreased performance across all metrics, though with
	      improved consistency in some cases. We observed metric-specific difficulties, particularly for
	      Availability Impact and Attack Complexity, which align with known human scoring challenges.
	      These findings highlight the complexities of integrating multiple data sources, the impact of
	      data imbalance, and the need for sophisticated approaches in automating vulnerability scoring.
