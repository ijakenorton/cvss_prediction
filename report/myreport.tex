...
\change[This next section is the continuation of the exploratory study with a focus on what are the]{In this section we explore the}
contributing factors for each metric / class for said metric. Clustering and analysis of the data
makes sense in many ways. As the data is already grouped into Common Weakness Enumeration~\cite{CWE}, it is likely that there are some sort of patterns we can find. As a somewhat arbitrary place to
start, choose K\notedme{Context not explained, and I'massuming should be $K$ anyway?} as 8 due to that being the number of CVSS metrics. For this I was hoping to see the
data would naturally have some clusters based on the general theme of the vulnerability /
description, e.g.\@ a cluster based generally around SQL injection / databases in general.\notedme{This paragraph needs elaboration: it's missing details at the moment.}
...
Below are the example topics gained from the initial kmeans\notedme{terminology should be consistent: above this is `K-means'} clustering:
\begin{itemize}
...
Here are some example of the descriptions, for your viewing pleasure:\notedme{a bit too informal, and had a grammatical error, but I fixed that.}
...
This\notedme{what is? Also all tables and figures should be floated top (t)} was a good indicator that it is worth looking into clustering further.
...
The methodology comprises several key steps:\notedme{You can't colon into subsections. Say something like "The methodology comprises several key steps, which are detailed in the subsections below." (AI actually made that quote, but I agree.)}
...
hyperparameters. In grid search attempts I found that the different coherences metrics\notedme{?} did not agree
with each other. In addition, as we are searching for unknown topics in a unsupervised fashion, it
is \change[dubious]{doubtful} how helpful these measures of fit are in any case. The hyperparameters below are broadly
...
\begin{figure}[h!]
	...
	\caption{Confidentiality Impact Topic Counts\notedme{Don't use subfigures like this: you can't repeat the same figure multiple times: again, just let {\LaTeX} do this stuff.}}
\end{figure}
...
Our analysis of the three CVSS impact metrics---Integrity, Confidentiality, and Availability---reveals intriguing patterns and distinctions in how different types of vulnerabilities are
