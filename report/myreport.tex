...
This report will try to answer these questions:\notedme{Typically expressed as `research questions'?}
...
Vulnerabilities are stored in a consistent system\notedme{system? Elaborate?} called Common Vulnerabilities and
...
{\footnotesize \notedme{Don't do unexplained formatting changes: use accepted semantic elements. I'd put this as an actual footnote to the subsection... noting that you're treating the subsection as a formatting command when it's semantic too. A key point of working in {\LaTeX} is that you should be focusing entirely on the semantics of your text, and not at all on the formatting, not least since changing the class file might make huge changes visually, and the class file should be driving how the PDF is generated.} Sourced from \href{https://nvd.nist.gov/vuln/detail/CVE-2024-38526}{NVD CVE-2024-38526
	Detail} \cite{polyfill}} \\
\bigskip

\change[This]{Each CVE report}\notedme{Don't use anaphoric references when you can be more specific.} has a unique identifier, which is given by one of the CVE numbering authorities (CNA~\cite{CNA}), such as
...
research~\cite{costa, nvd_example1, nvd_example2}.  This makes a lot of sense\notedme{what makes a lot of sense?} as it is built
...
This process is inherently \change[inaccurate]{imprecise}\notedme{check, but I'm pretty sure you mean `imprecise', not `inaccurate'.}, it is not a system designed to give precise scores. If we
look at~\ref{formula} above, the formula (\ref{equation} shows the CVSS 3.1 base score formula for
reference) which produces the score, does not match exactly the experts decision.\notedme{wrong refs} There is a lot of
rounding and approximation going on. This is designed to make a system which is easy to use and
quick to complete by security professionals. There is a space\notedme{sounds informal, and ambiguous to me: do you mean `role'? Also, this is the conclusion of Section 2, so make that clearer in the text. What you're writing here is not just a passing remark, it's you justifying flow into the next section\dots} for CVSS, however this along with the
...
by Johnson et al. entitled \notedme{make sure your quotes are all ``fancy'' ones and not "straight" ones, in your body text.} "Can the Common Vulnerability Scoring System be Trusted? A Bayesian
...
The scorers for both NVD and MITRE do rate CVEs reasonably \change{similarly}. One pattern you can see as shown
by Figure~\ref{fig:counts}, is that NVD generally give\notedme{grammar} the most common categorical output more ratings.
...
my \change{concerns regarding} these results, not that they are wrong per se, but that they do not really tell us
...
% 	\caption{\label{fig:purity_integrity}The purity score for when the topics have been assigned with a focus on \texttt{Integrity Impact}, split by the
...
\todo[inline]{Work in progress, copy paste of previous but with the new graphs currently}
...
\begin{itemize}
	...
	\item Predominant terms include "service", "function", "crash", "crafted", "remote", "denial",
	      "cause", "attackers", "memory", and "dos".

	\item The presence of terms like "denial" and "dos" (denial of service) aligns with
...
\begin{itemize}
	...
	\item Predominant terms include "scripting", "xss", "site", "stored", "arbitrary", "user",
	      "web", "inject", "html", and "css".
...
\begin{itemize}
	...
	\item Predominant terms include "execution", "execute", "crafted", "remote", "upload", "code",
	      "arbitrary", "file", "request", and "attackers".

	\item The presence of terms like "remote" and "arbitrary" suggests vulnerabilities that allow
...
\begin{itemize}
	...
	\item Predominant terms include "inject", "arbitrary", "html", "cross", "scripting", "site",
	      "xss", "user", "stored", and "web".
...
\begin{itemize}
	...
	\item Predominant terms include "compromise", "oracle", "access", "data", "cvss", "attacks",
	      "unauthorized", "successful", "accessible", and "base".
...
\begin{itemize}
	...
	\item Predominant terms include "attackers", "arbitrary", "injection", "execution", "commands",
	      "execute", "function", "command", "remote", and "code".

	\item The presence of terms like "remote" and "arbitrary" coupled with "execution" and "command"
...
system availability.\todo[inline]{talk about an example of a bad case}
...
\begin{itemize}
	...
	\item \textbf{Denial of Service:} Terms related to denial of service (DoS) attacks are prominent
	      in the NONE category for both Integrity and Confidentiality impacts. However, their absence
	      from the Availability impact categories, particularly in the HIGH category where we might
	      expect them, highlights the data imbalance issue in the Availability metric.
